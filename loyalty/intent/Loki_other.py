...
def getResult(inputSTR, utterance, args, resultDICT, refDICT):
    ...
    if utterance == "[對方][會]載其他[女生][朋友]回家":
        if CHATBOT_MODE:
            if args[0] not in ("我"):
                resultDICT["response"] = getResponse(utterance, args).format(*args)
        else:
    ...
    if utterance == "[對方]不讓[我]看[他][LINE]":
        if CHATBOT_MODE:
            if args[0] not in ("我") and args[3] in userDefinedDICT["_sm"]:
                resultDICT["response"] = getResponse(utterance, args).format(*args)
        else:
            # write your code here
            pass
    ...
    if utterance == "[對方]跟[別][人]去[旅館]":
        if CHATBOT_MODE:
            if args[0] not in ("我") and args[3] in userDefinedDICT["_hotel"]:
                resultDICT["response"] = getResponse(utterance, args).format(*args)
        else:
            # write your code here
            pass
